...
import edu.kit.ipd.pronat.audio.AudioFormatMatcher;
import edu.kit.ipd.parse.luna.tools.ConfigManager;
import edu.kit.ipd.pronat.revise.support.EmptyMap;
...
public class MultiASR implements IMultiASR {
	...
	public MultiASR() {
		...
		try {
			tempFolder = Files.createTempDirectory("MultiASR");
			...
			ffmpeg = new FFmpeg(ffmpegPath.toString());
		} catch (final IOException e) {
		...
		ffprobe = new FFprobe(ffprobePath.toString());
	...
	private Path convertAudio(AudioFormat targetFormat, Path source) throws IOException {
		...
		final FFmpegBuilder ffmpegb = ffmpeg.buider();
		...
		final FFmpegJob job = ffmpegExecutor.createJob(ffmpegb);
	...
	public List<ASROutput> recognize(Path sourceAudio, Map<String, String> requiredCapabilites, Map<String, String> optionalCapabilites) {
		...
		final List<ASROutput> results = new ArrayList<>(asrs.size());
		...
		for (final IASR asr : asrs) {
			...
			for (final AudioFormat format : asr.getSupportedAudioFormats()) {
				if (audioFormatMatcher.match(sourceFormat, format)) {
					try {
				...
				try {
					converted = convertAudio(format, sourceAudio);
				} catch (final IOException e) {
					e.printStackTrace();
				}
				if (converted != null) {
		...
		for (final IPostProcessor pp : pps) {
